...
for (i in 1:N_mcmc) {
  ...
  m_roc[,i] <- coords(roc_res, x=spec, input=c('specificity'), ret=c('sensitivity'))
}

names(ms)
length(ms$q[,1])

roc_res <- roc(d$Y, ms$q[N_mcmc,])
auces[N_mcmc] <- as.numeric(roc_res$auc)
m_roc[N_mcmc] <- coords(roc_res, x=spec, input=c('specificity'), ret=c('sensitivity'))
