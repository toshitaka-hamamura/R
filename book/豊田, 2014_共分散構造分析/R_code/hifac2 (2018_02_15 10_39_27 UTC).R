hifac2<-function(x,y,emethod='GLS',歪み1=TRUE,歪み2=TRUE)
{
  df=2
  if (歪み1) {df<-df-1}
  if (歪み2) {df<-df-1}
  ...
  if (歪み1 | 歪み2) {
     if (歪み1) {
         s母数[6]<-sx3-(s母数[2]^3)*s母数[1]
         if (歪み2) {s母数[7]<-sy3-(s母数[3]^3)*s母数[1]}
     }else {s母数[6]<-sy3-(s母数[3]^3)*s母数[1]}
  }
  #標本積率のセット
  s<-matrix(0,7,1); 
  ...
  目的<-function(母数)
  {
    ...
    sigthe[2]<-母数[2]*母数[3];
    sigthe[3]<-母数[3]^2+母数[5];
    if (歪み1) {sigthe[4]<-(母数[2]^3)*母数[1]+母数[6]}
       else    {sigthe[4]<-(母数[2]^3)*母数[1]}
    sigthe[5]<-(母数[2]^2)*母数[3]*母数[1];
    sigthe[6]<-母数[2]*(母数[3]^2)*母数[1];
    if (歪み2) {
        if (歪み1) {sigthe[7]<-(母数[3]^3)*母数[1]+母数[7]
        }else      {sigthe[7]<-(母数[3]^3)*母数[1]+母数[6]}
    }else {sigthe[7]<-(母数[3]^3)*母数[1]}
  ...
  if (歪み2) {
      if (歪み1)  {q21<-c('f:3次','f->x係数','f->y係数','x誤差2次','y誤差2次','x誤差3次','y誤差3次')
      }else      {q21<-c('f:3次','f->x係数','f->y係数','x誤差2次','y誤差2次','y誤差3次')}
  }else if(歪み1) {q21<-c('f:3次','f->x係数','f->y係数','x誤差2次','y誤差2次','x誤差3次')
  }else {q21<-c('f:3次','f->x係数','f->y係数','x誤差2次','y誤差2次')}
